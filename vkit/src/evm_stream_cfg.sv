...
class evm_stream_cfg extends evm_object;
    ...
    //==========================================================================
    bit    buffered_mode = 1;                  // Use buffered streaming (dynamic generation)
    int    buffer_size = 1024;                 // Number of samples per buffer
    int    buffer_refill_threshold = 256;      // Refill when buffer has this many samples left
    string temp_buffer_file = "temp_buffer.txt"; // Temporary file for buffer generation
    ...
    // Get command for buffered generation (with sample offset)
    function string get_python_buffer_cmd(int start_sample, int num_samples);
        string cmd;
        if (python_gen_script != "") begin
            cmd = $sformatf("python %s --type %s --freq %.0f --fs %.0f --amp %.3f --start %0d --count %0d --channels %0d --output %s",
                python_gen_script, signal_type, signal_freq_hz, sample_rate_hz, 
                signal_amplitude, start_sample, num_samples, num_channels, temp_buffer_file);
        end
        return cmd;
    endfunction
